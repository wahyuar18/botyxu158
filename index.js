...
limitawal = 30
memberlimit = 0
cr = '*_YXU BOT SUDAH TERVERIFIKASI_*'
/*************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr already scan.WELCOME  IN','white'),color('YXU BOT','red'),color('•','white'),color('WahyuuAR18','yellow'))
})
            ...
            if (bad.includes(messagesC)) {
				...
				if (messagesC.includes("://chat.whatsapp.com/")){
					...
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
						reply(`*「 LINK GROP DETECTED 」*\nSorry ngab ${sender.split("@")[0]} anda akan di kick dari group 5detik lagi`)
						setTimeout( () => {
            ...
            case 'bitly':
               ...
               case 'help': 
         	   ...
         	   case 'creator':
                  ...
                  client.sendMessage(from, 'Langsung chat aja ngab owner nya gans:v',MessageType.text, { quoted: mek} )
					break    
				case 'profile':
    				client.updatePresence(from, Presence.composing)
    				if (!isRegistered) return reply(ind.noregis())
                    if (isLimit(sender)) return reply(ind.limitend(pusname))
    				try {
    	            profil = await client.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				    } catch {
...
• Limit: ${limitawal-updated.limit}`
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('𝗸𝗶𝗿𝗶𝗺 𝗳𝗼𝘁𝗼 *dengan* 𝗰𝗲𝗽𝘁𝗶𝗼𝗻 ${prefix}𝗼𝗰𝗿')
					}
					await limitAdd(sender)
				break
				case 'stiker': 
 			 ...
 			 //  reply('p\n' + util.format({fontPath, inputPath, outputPath, tgl, hari, textnulis}))
				...
				if (!isQuotedSticker) return reply('𝗥𝗲𝗽𝗹𝘆/𝘁𝗮𝗴 𝘀𝘁𝗶𝗰𝗸𝗲𝗿 !')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '*Succes*'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
				break 
				...
				try {
						...
						console.log(`Error :`, color(e,'red'))
						reply('[ *EROR* ] 𝗸𝗲𝗺𝘂𝗻𝗴𝗸𝗶𝗻𝗮𝗻 *username* 𝘁𝗶𝗱𝗮𝗸 *valid*')
					}
					await limitAdd(sender)
				break
				//group feature 
           ...
           case 'demote':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠𝗲𝗻𝗼𝗻𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 𝗳𝗶𝘁𝘂𝗿 *welcome*/𝗹𝗲𝗳𝘁 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻𝗶️')
					} else {
						reply(ind.satukos())
					}
					break 
				...
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠𝗲𝗻𝗼??𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 𝗳𝗶𝘁𝘂𝗿 𝗻𝘀𝗳𝘄 𝗱𝗶 𝗴𝗿𝗼𝗽 𝗶𝗻𝗶️')
					} else {
						reply(ind.satukos())
					}
				break
